...
{
    if (!RHSPIDriver::init()) {
        Serial.println("Can't init RHSPI driver");
	    return false;
    }
    ...
    if (!setupInterruptHandler()) {
        Serial.println("Can't init setup interrupt handler");
	    return false;
    }
    ...
    uint8_t returnByte = spiWrite(RH_RF95_REG_01_OP_MODE, RH_RF95_MODE_SLEEP | RH_RF95_LONG_RANGE_MODE);
    ...
    {
        Serial.println("Maybe spi read is kil :(?");
	Serial.println(spiRead(RH_RF95_REG_01_OP_MODE), HEX); // FF
